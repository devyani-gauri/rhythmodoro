...
def estimate_song_count(
    ...
    *,
    ...
    """Return only the time estimate and recommended song count without running search.

    Returns dict with keys: minutes_estimate, avg_song_minutes, recommended_count
    """
    minutes_estimate = 0
...
def run_playlist(
    ...
    *,
    ...
    if minutes_estimate > 0 and avg_song_minutes:
        ...
        recommended_count = default_count
    # Effective target count (override if provided and valid)
    effective_target_count = None
    if isinstance(desired_song_count, int) and desired_song_count > 0:
        effective_target_count = max(1, min(max_playlist_songs, desired_song_count))
    else:
        effective_target_count = recommended_count
    ...
    fetch_k = min(max(effective_target_count * fetch_multiplier, effective_target_count), MAX_QUERY_WINDOW)
    ef = max(64, fetch_k + 16)  # strictly larger than k to satisfy Milvus requirement
    ...
    # Post-process
    max_blended = int(round(blended_max_ratio * effective_target_count)) if include_blended else 0
    final: List = []
    blended_used = 0
    ...
    if use_time_budget:
        ...
        if not final and len(raw_results) > 0:
            ...
            if top_dur <= target_ms + budget_overshoot_ms:
                final = [top]
        # If a desired count was provided, adjust list size accordingly.
        if isinstance(desired_song_count, int) and desired_song_count > 0:
            target_len = min(max_playlist_songs, desired_song_count)
            if len(final) > target_len:
                final = final[:target_len]
            elif len(final) < target_len:
                # Append more ignoring time budget but respecting blended cap and max songs
                for hit in raw_results:
                    if hit in final:
                        continue
                    this_vibe = hit.entity.get("vibe") or ""
        ...
        for hit in raw_results:
            ...
            if is_blended:
                blended_used += 1
            if len(final) >= effective_target_count:
                break
